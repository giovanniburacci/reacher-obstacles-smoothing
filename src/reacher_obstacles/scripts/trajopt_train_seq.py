...
# Example RM sequence:
# targets = [
#     np.array([ 0.18,  0.00, 0.015]),
#     np.array([-0.12, -0.18, 0.015]),
#     np.array([-0.18,  0.18, 0.015]),
# ]
# targets = [np.array([*CONFIGS[config]['target'], 0.015])]  # default single target
...
print(f"Targets (sequential): {targets}")

# --- Sequential TrajOpt over RM-style subgoals ---
...
np.savez(
    ...
    X=X, A=A, U=U,
    targets=np.array(targets), # New array stored for targets
    phase_lengths=phase_lengths # New array stored for phases length
)
...
# Build per-step active-target index from phase_lengths
